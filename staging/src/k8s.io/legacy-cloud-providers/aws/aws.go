...
// sortELBSecurityGroupList returns a list of sorted securityGroupID based on it's original order built by buildELBSecurityGroupList
// The logic is
//	* securityGroups specified by ServiceAnnotationLoadBalancerSecurityGroups appears first in order and
...
func (c *Cloud) updateInstanceSecurityGroupsForLoadBalancer(lb *elb.LoadBalancerDescription, instances map[InstanceID]*ec2.Instance, annotations map[string]string) error {
	...
	klog.Warningf("[yyyng] before sort: %v", lbSecurityGroupIDs)
	c.sortELBSecurityGroupList(lbSecurityGroupIDs, annotations)
	klog.Warningf("[yyyng] after sort: %v", lbSecurityGroupIDs)
